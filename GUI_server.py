...
class GUIServerConfig:
            ...
            if not match:
                ...
                for ip_byte in ip_bytes:
                    if int(ip_byte) < 0 or int(ip_byte) > 255 or any(i.isalpha() for i in ip_byte):
                        ...
                        self.ip_address_v = True
                self.message += "IP Address: " + ip_address + "\n"
    ...
    def start_server(self):
        ...
        if self.lease_v and self.name_v and self.ip_address_v and self.mask_v:
            gui_server.run(self.message)
